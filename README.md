...
| **验证看板** | 因子库浏览，查看各因子 IC/IR/单调性指标数值 |
...
Web 端验证看板以因子列表形式展示：每条因子可展开查看 IC 均值、IC IR、单调性得分等关键指标数值。图表组件（IC 时序、分布、衰减、分组收益、多空净值）已实现于 `web/components/charts.py`，目前尚未接入看板页面。

### 回测输出

在 Web 回测页面运行后，产出以下图表：

- **净值曲线 (NAV Curve)** — 策略累计净值走势
- **回撤曲线 (Drawdown)** — 历史回撤序列，标注最大回撤点
- **分组净值曲线 (Group NAV Curves)** — 各分组（如 Q1-Q5）的累计净值，检验单调性
- **绩效统计表 (Performance Statistics)** — 年化收益、年化波动率、夏普比率、最大回撤、Calmar 比率、胜率、Sortino 比率等
